...
`stratification_height_tolerances.m` records the repeat-specific epsilon values reported in the thesis, including the corrected second full-height 30 cm^3/min value of 0.095 kg m^-3. These functions are a clean reference implementation of the documented method, not a recovered copy of the historical production script. The original data-set crops, reference profiles, time origins and completed run configurations still need to be retained with the source data for exact historical reproduction. Do not pass the thesis's kg m^-3 tolerances to the intensity-normalisation workflow in `densityheight.m`.
